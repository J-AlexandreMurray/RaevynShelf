...
const KEY='raevyntide_mvp_books';
...
export async function library(uid){if(!supabase)return{display_name:'My RaevynTide'};const {data,error}=await supabase.from('libraries').select('id,display_name,slug,is_public').eq('user_id',uid).maybeSingle();if(error)throw error;return data||{display_name:'My RaevynShelf'}}
export async function saveLibrary(uid,name){const value=name.trim()||'My RaevynTide';if(!supabase)return{display_name:value};const {data,error}=await supabase.from('libraries').upsert({user_id:uid,display_name:value},{onConflict:'user_id'}).select().single();if(error)throw error;return data}
...
export async function createOrRefreshStatCard(userId, displayName, theme = "midnight") {
  ...
  if (!libraryRow?.id) {
    libraryRow = await saveLibrary(userId, displayName || "My RaevynTide");
  }
  ...
  const payload = {
    ...
    display_name: (displayName || libraryRow.display_name || "My RaevynTide").trim(),
    theme,
